...
## Using Ollama (Local LLM - No API Keys Needed)

To run ADK with a local Ollama instance, use the `ollama_chat/` prefix:

```python
from google.adk import Agent
from google.adk.models.lite_llm import LiteLlm

# Point ADK to your local Ollama instance
local_llm = LiteLlm(model="ollama_chat/gemma3:latest")

# Define your agent with the local model
root_agent = Agent(
    name="LocalHelperAgent",
    description="An autonomous agent running entirely on local hardware.",
    model=local_llm,
    tools=[]  # Add your Python functions or MCP tools here
)

# Execute a request
response = root_agent.run("What are the primary benefits of keeping LLM inference local?")
print(response)
```

**Available Ollama models:** `gemma3:latest`, `mistral`, `llama2`, `neural-chat`, etc.

**Requirements:**
- Ollama running locally: `ollama serve`
- Model pulled: `ollama pull gemma3:latest`

Everything else in the agent stays exactly the same. The tools, the filesystem MCP server, and the board are all model-agnostic, so only the one model line changes.
